...
void documentShared::insertFromMimeData(const QMimeData* source) {
...
void documentShared::paintEvent(QPaintEvent *event) {
...
int documentShared::calculateWidthForUsername(QString& username) {
...
int documentShared::calculateOffsetY(int curFontSize) {
    int retVal = 0;
...
int documentShared::calculateOffsetHeight(int curFontSize) {
    int retVal;
...
int documentShared::getAlignmentValues(QVector<std::pair<int,int>>& alignments) {
...
QString documentShared::hexToHtmlRGB(QString hexARGB) {
...
void documentShared::setBtnColorChecked(bool val) {
    this->isColorBtnChecked = val;
}

void documentShared::setAlignmentsVector(QVector<std::pair<int,int>> alignmentVector) {
    this->alignments = alignmentVector;
}

void documentShared::setEditorColor(QString color) {
    this->editorColor = color;
}

void documentShared::addRemoteCursor(QString username, std::pair<QString, int> colorAndPos) {
    this->remoteCursors.insert(username, std::make_pair(colorAndPos.first, colorAndPos.second));
}

QString documentShared::getEditorColor() {
...
QHash<QString, std::pair<QString,int>> documentShared::getRemoteCursors() {
    return this->remoteCursors;
}

bool documentShared::isBtnColorChecked() {
    return this->isColorBtnChecked;
}

QVector<std::pair<int,int>> documentShared::getAlignmentsVector() {
    return this->alignments;
}

QString documentShared::replaceColorWithEditorColor(QString text) {
    QString html = text;
...
QStringList documentShared::getRegexListFromHTML(QString text, QRegularExpression rx) {
    QStringList list;
...
void documentShared::removeRemoteCursor(std::string username) {
    remoteCursors.remove(QString::fromStdString(username));
    this->viewport()->update();
}

void documentShared::changeRemoteCursor(QString username, QString color, int pos) {
    //color[1] = 'f';
    //color[2] = 'f';
    addRemoteCursor(username, std::make_pair(color, pos));
...
void documentShared::hideHorizontalRect() {
    showHorizontalRect = false;
    this->viewport()->update();
}
